...
public class LC105ConstructBinaryTreeFromPreoderandInorder {
    class SolutionRecur {
        int pre_idx = 0;
        ...
        public TreeNode buildTree(int[] preorder, int[] inorder) {
            ...
            return recur(0, inorder.length, preorder, inorder);
        }

        TreeNode recur(int in_left, int in_right, int[] preorder, int[] inorder) {
            if (pre_idx > preorder.length || in_left == in_right) {
                return null;
            }
            TreeNode root = new TreeNode(preorder[pre_idx]);
            int idx = map.get(preorder[pre_idx]);
            pre_idx++;
            root.left = recur(in_left, idx, preorder, inorder);
            root.right = recur(idx + 1, in_right, preorder, inorder);
            return root;
